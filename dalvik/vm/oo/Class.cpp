    ...
    const u1* pEncodedData, Object* classLoader)
{
    ...
    if (pHeader->instanceFieldsSize != 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (gDvm.shouldCARWorking && dvmHasCARMemberAnnotation(&(newClass->ifields[i]).field)) {
				//LOGD("field has CARMember %s", (newClass->ifields[i]).field.name);
...
static bool computeFieldOffsets(ClassObject* clazz)
{
    ...
    for (i = 0; i < clazz->ifieldCount; i++) {
        ...
        /*
        if (strcmp(pField->signature, "Ljava/lang/Delegates;") == 0 || \
            strcmp(pField->signature, "Ljava/lang/Callbacks;") == 0) {
        */
